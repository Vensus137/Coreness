...
class TableOperations:
    ...
    def __init__(self, db_service, engine, db_type: str, metadata_cache: TableMetadataCache, logger, formatter):
        """
        Инициализация операций с таблицами
        """
    ...
    def create_table(self, table_class: Any) -> bool:
        ...
        try:
            ...
            self.formatter.print_info(f"Создаю таблицу {table.name}...")
            ...
            if table.name in existing_tables:
                self.formatter.print_success(f"Таблица {table.name} успешно создана")
                return True
            else:
                self.formatter.print_error(f"Ошибка: таблица {table.name} не создалась")
                return False
                
        except Exception as e:
            self.formatter.print_error(f"Ошибка создания таблицы {table_class.__table__.name}: {e}")
            raise
    ...
    def recreate_table(self, table_class: Any) -> bool:
        ...
        try:
            table = table_class.__table__
            self.formatter.print_warning(f"Удаляю таблицу {table.name}...")
            table.drop(self.engine, checkfirst=True)
            self.formatter.print_info(f"Создаю таблицу {table.name}...")
            table.create(self.engine, checkfirst=True)
            self.formatter.print_success(f"Таблица {table.name} успешно пересоздана")
            return True
        except Exception as e:
            self.formatter.print_error(f"Ошибка пересоздания таблицы {table_class.__table__.name}: {e}")
            raise
    ...
    def recreate_table_with_data(self, table_class: Any) -> bool:
        ...
        try:
            ...
            with self.engine.begin() as conn:
                ...
                for row in rows:
                    ...
                    try:
                        ...
                        self.logger.warning(f"Пропущена запись из-за нарушения {violation_type}: {e}")
                ...
                if self.db_type == 'postgresql':
                    ...
                    self.formatter.print_info("SQLite: переименование constraint/sequence не требуется")
            
            # Выводим summary о пропущенных записях
            if skipped_records > 0:
                self.formatter.print_warning(
                    f"⚠️  Пропущено записей из-за нарушения constraints: {skipped_records}"
                )
                ...
                if unique_violations_by_column:
                    ...
                    self.formatter.print_warning(f"   UNIQUE constraints: {unique_count} ({violations_summary})")
                ...
                if fk_violations_by_column:
                    ...
                    self.formatter.print_warning(f"   Foreign Keys: {fk_count} ({violations_summary})")
            ...
            self.formatter.print_error(f"Ошибка при перезаливке таблицы {table_class.__table__.name}: {e}")
            raise
    ...
    def drop_table(self, table_name: str) -> bool:
        ...
        try:
            ...
            if table_name not in existing_tables:
                self.formatter.print_warning(f"Таблица {table_name} не найдена")
                return False
            
            with self.engine.connect() as conn:
                self.formatter.print_warning(f"Удаляю таблицу {table_name}...")
                conn.execute(text(f'DROP TABLE IF EXISTS "{table_name}"'))
                conn.commit()
            self.formatter.print_success(f"Таблица {table_name} успешно удалена")
            return True
            
        except Exception as e:
            self.formatter.print_error(f"Ошибка при удалении таблицы {table_name}: {e}")
            return False
            ...
            with self.engine.connect() as conn:
                for col_name in unique_columns:
                    if col_name not in existing_unique_indexes:
                        ...
                        try:
                            self.formatter.print_info(f"Создаю UNIQUE constraint для колонки {col_name}...")
                            if self.db_type == 'sqlite':
                            ...
                            conn.commit()
                            self.formatter.print_success(f"UNIQUE constraint для колонки {col_name} создан")
                        except Exception as e:
                            self.logger.warning(f"Ошибка создания UNIQUE constraint для {col_name}: {e}")
                            ...
                            try:
                                ...
                                conn.commit()
                                self.formatter.print_success(f"UNIQUE индекс для колонки {col_name} создан (альтернативный способ)")
                            except Exception as e2:
                                self.logger.error(f"Не удалось создать UNIQUE constraint для {col_name}: {e2}")
            
            return True
            
        except Exception as e:
            self.logger.warning(f"Ошибка при проверке UNIQUE constraints: {e}")
            return False
