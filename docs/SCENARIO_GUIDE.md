...
Результат выполнения ограничения
...
Данные, возвращаемые сервисом в поле response_data
...
```
- Удалены ссылки (режим cleanup):
```yaml
invite_links_deleted:
- https://t.me/...
```
- Ошибка: `null`
...
Ответ содержит только текст запроса пользователя (request_text)

Варианты:
- Только текст запроса пользователя:
```yaml
request_text: Текст обращения пользователя
```
...
Ответ содержит готовый текст для отправки
...
Поддерживаемые типы правил для поля rules:
  ...
  - contains: значение поля должно содержать value (строка)
  - starts_with: значение поля должно начинаться с value (строка)
