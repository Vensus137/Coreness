...
#### User Storage
Аналогичное управление хранилищем данных пользователей с теми же возможностями.
...
**Проверка доступа:** сценарий `access_check` выполняется перед каждой операцией
...
#### Команда `/me`
Публичная команда для просмотра информации о себе:
- ID пользователя
- Username
- Имя и фамилия
- Язык интерфейса
...
#### Команда `/file`
Скрытая команда для разработчиков:
- Получение `file_id` вложений
- Получение `type` вложений
- Для последующей пересылки файлов

#### Рассылки (для администраторов)
Команда `/mail` для массовой рассылки сообщений пользователям.

## 🛡️ Безопасность

- **Валидация доступа:** проверка прав перед каждой операцией
