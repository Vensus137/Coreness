...
📋 **Подробная архитектура проекта:** [Архитектура проекта](ARCHITECTURE.md)
...
└── services/          # Бизнес-сервисы
    ...
    └── extensions/    # Расширения сервисов (не входят в базовую версию платформы)
```
...
📋 **Концептуальное описание слоев:** [Архитектура проекта - Слои и их назначение](ARCHITECTURE.md#слои-и-их-назначение)

### 🚀 Сервисы (Services)
- **Назначение**: Основная бизнес-логика приложения
- **Особенности**:
  - Работают автономно и асинхронно
  - Могут иметь опциональный метод `run()` для фонового запуска
  - Могут зависеть от утилит (foundation, level-слои, core, database)
...
- **Особенности**:
  ...
  - Размещаются в `plugins/utilities/extensions/` или `plugins/services/extensions/`
  - Позволяют добавлять новый функционал без изменения базовой платформы
...
### Custom Layers — Когда использовать?
**Используйте для:**
...
- Автономных сервисов с методом `run()`
...
1. Скопировать папку плагина в `plugins/utilities/extensions/` или `plugins/services/extensions/`
2. Убедиться, что в папке есть `config.yaml`
...
**⚠️ Важно:** Файл `config.yaml` является **обязательным** для всех плагинов. Без него плагин не будет обнаружен системой и не сможет работать.

### 2. Определение типа плагина
Тип плагина определяется **автоматически** по расположению:
- Если плагин находится в `plugins/utilities/` → **утилита**
- Если плагин находится в `plugins/services/` → **сервис**
...
└── services/
    ...
    └── extensions/
        ...
        └── ai_rag_service/   # Extension сервис (RAG функционал)
```

## Конфигурация плагина (config.yaml)
...
📋 **[Шаблоны YAML-конфигов для сервисов и утилит](PLUGIN_CONFIG_GUIDE.md)**
...
📋 **Иерархия зависимостей и правила:** [Архитектура проекта - Иерархия зависимостей](ARCHITECTURE.md#иерархия-зависимостей)
...
optional_dependencies:
  ...
  - "analytics_service"   # Опциональная зависимость
```

**Примечание:** Опциональные зависимости не влияют на порядок инициализации и не вызывают ошибок при отсутствии.

## Инициализация и порядок загрузки

### Алгоритм инициализации
1. **Сканирование** всех плагинов рекурсивно
2. **Построение графа зависимостей**
3. **Проверка циклических зависимостей**
4. **Топологическая сортировка** утилит
5. **Инициализация утилит** в порядке зависимостей
6. **Инициализация сервисов** (в любом порядке)
...
7. chat_service (зависит от logger, database_service, event_processor)
```
...
if foundation_issues:
    print("❌ FOUNDATION УТИЛИТЫ ИСПОЛЬЗУЮТ ДРУГИЕ УТИЛИТЫ:", foundation_issues)
```
...
1. Создать папку в соответствующей директории (foundation/core/database/level_N/services)
2. Добавить `config.yaml` с описанием
3. Реализовать код плагина
4. Перезапустить приложение

### Изменение зависимостей
1. Обновить `config.yaml`
2. Проверить отсутствие циклических зависимостей
3. Убедиться в соблюдении иерархии зависимостей
4. Перезапустить приложение

### Удаление плагина
1. Удалить папку плагина
2. Убрать зависимости из других плагинов
3. Перезапустить приложение