...
- [Триггеры (trigger)](#-триггеры-trigger)
- [Scheduled сценарии (запуск по расписанию)](#-scheduled-сценарии-запуск-по-расписанию)
- [Действия (step)](#-действия-step)
  - [Кэширование данных в сценариях](#кэширование-данных-в-сценариях)
  - [Сохранение контекста между сценариями](#сохранение-контекста-между-сценариями)
- [Переходы (transition)](#-переходы-transition)
- [Плейсхолдеры (Placeholders)](#-плейсхолдеры-placeholders)
  - [Синтаксис плейсхолдеров](#синтаксис-плейсхолдеров)
  - [Доступные данные](#доступные-данные)
...
- И другие поля события (см. [EVENT_GUIDE.md](./EVENT_GUIDE.md))
...
В scheduled сценариях доступны специальные поля события. Подробное описание всех полей см. в разделе [Поля scheduled сценариев](./EVENT_GUIDE.md#поля-scheduled-сценариев) в EVENT_GUIDE.md.
...
<tr><td><code>length</code></td><td>Подсчет длины</td><td><code>{text|length}</code></td><td><code>15</code> (количество символов)</td></tr>
<tr><td><code>length</code></td><td>Подсчет длины массива</td><td><code>{array|length}</code></td><td><code>3</code> (количество элементов)</td></tr>
...
<tr><td><code>not_ready</code></td><td>Проверка что действие еще выполняется</td><td><code>{_async_action.ai_req_1|not_ready}</code></td><td><code>true</code> если выполняется, <code>false</code> если готово</td></tr>
</table>
