...
- [ai_service](#ai_service) (6 действий)
...
- [tenant_hub](#tenant_hub) (4 действий)
...
- **`tenant_id`** (`integer`, обязательное, min: 1) — ID тенанта (обязательно)
...
- **`bot_id`** (`integer`, обязательное, min: 1) — ID бота
...
- **`bot_id`** (`integer`, обязательное, min: 1) — ID бота
- **`delete_message_id`** (`integer`, опционально, min: 1) — ID сообщения для удаления. Если не указан, используется message_id из контекста события. Chat ID по умолчанию берется из события (chat_id), если не указан явно
...
- **`message_reply`** (`integer`, опционально, min: 1) — ID сообщения для ответа
...
- **`_response_key`** (`string`) (опционально) — Кастомное имя для основного поля результата (помеченного 🔀). Если указан, основное поле будет сохранено в `_cache` под указанным именем вместо стандартного. Доступ через `{_cache.{_response_key}}`. Работает только для действий, которые поддерживают переименование основного поля.
...
- **`invoice_id`** (`integer`, обязательное, min: 1) — ID инвойса (обязательно)
...
- **`tenant_id`** (`integer`, обязательное, min: 1) — ID тенанта
- **`bot_id`** (`integer`, обязательное, min: 1) — ID бота
...
- **`tenant_id`** (`integer`, обязательное, min: 1) — ID тенанта
- **`bot_id`** (`integer`, обязательное, min: 1) — ID бота
- **`target_user_id`** (`integer`, опционально, min: 1) — ID пользователя для создания инвойса (опционально, по умолчанию user_id из события)
...
- **`amount`** (`integer`, обязательное, min: 1) — Количество звезд (обязательно, целое число)
...
- **`target_user_id`** (`integer`, опционально, min: 1) — ID пользователя для получения инвойсов (опционально, по умолчанию user_id из события)
...
- **`count`** (`integer`, обязательное, min: 1) — Количество элементов для выбора (без повторений)
...
- **`count`** (`integer`, обязательное, min: 1) — Количество чисел для генерации
- **`seed`** (`any`, опционально) — Seed для детерминированной генерации (опционально, может быть числом, строкой или другим типом)
...
- **`seconds`** (`float`, обязательное, min: 0.0) — Количество секунд задержки (можно использовать дробные значения, например 0.5 или 22.5)
...
- **`tenant_id`** (`integer`, обязательное, min: 1) — ID tenant'а (передается автоматически из контекста)
...
- **`timeout`** (`integer`, опционально, min: 0.0) — Таймаут ожидания в секундах (опционально). При истечении таймаута wait_for_action возвращает ошибку timeout, но выполнение сценария продолжается. Основное асинхронное действие продолжает выполняться в фоне даже после таймаута.
...
- **`user_id`** (`integer`, обязательное, min: 1) — ID пользователя Telegram
- **`tenant_id`** (`integer`, обязательное, min: 1) — ID тенанта
...
- **`expires_in_seconds`** (`integer`, опционально, min: 0) — Время истечения в секундах (None или 0 = навсегда, устанавливается 3000 год)
